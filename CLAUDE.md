...
- **业务点 SetDi(ts=0) 污染 data_recorder**（H8）：采集模块 offline 分支写 pt=1 时用 ts=0，虽 pt=1 因 bug#2 修复被 EventBus 屏蔽，但若未来任何代码给业务点传 ts=0 会让 rt_status/di_log 出现 ts_ms=0 行。防御修复：SetDi 中 pt≠1 && ts==0 自动填 NowMs()。
- **`DataRecorder::TimerThread` mysqlMtx_ 长时间阻塞订阅者**（H10）：老代码把 `mysql_ping + DisconnectMySQL + ConnectMySQL + CreateTables` 都在 mysqlMtx_ 锁里，重连若耗数秒到 30 秒，其间 OnDIChange/OnAIChange 等订阅者被阻塞，EventBus Publish 也随之停摆。修复：mysql_ping 快速检测在锁内、真正重连在锁外用独立 MYSQL* 建立、成功后进锁 swap；关闭旧连接在锁外。锁临界区从"几十秒重连"降到"一次原子交换"。无测试覆盖（需真 MySQL 做断连场景），仅编译验证 + 代码 review。
